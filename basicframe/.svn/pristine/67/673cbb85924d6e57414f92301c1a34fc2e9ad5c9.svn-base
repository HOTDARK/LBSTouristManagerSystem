/*
 * Copyright © 2015, Wangding (Chengdu) Tech Co.,Ltd. 
 * All right reserved.
 */
package com.hd.workflow.app.task;

import com.hd.workflow.engine.pvm.VariableScope;
import com.hd.workflow.engine.pvm.delegate.TaskDelegate;

/**
 * TODO 类描述
 * @version	0.0.1
 * @author	<a href="mailto:sunjian@iwangding.com">孙剑</a>
 * @date	2015-3-5 下午3:11:14
 */
public class SayHelloTask implements TaskDelegate {

	@Override
	public void execute(VariableScope scope) {
		System.out.println("hello");
		scope.setVariable("aaa_password", "123");
		System.out.println(scope.getVariable("crm_username"));
	}

}
 