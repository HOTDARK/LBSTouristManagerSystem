			...
			try {
				...
				for (int i = 0; i < fslist.length; i++) {
					...
					case 2: // TYPE_LOCAL_DISK : 本地硬盘
						...
						disk.setTotalSize(new BigDecimal(usage.getTotal()).divide(divi, 2, BigDecimal.ROUND_HALF_UP));
						disk.setAvailSize(new BigDecimal(usage.getAvail()).divide(divi, 2, BigDecimal.ROUND_HALF_UP));
						disk.setUsedSize(new BigDecimal(usage.getUsed()).divide(divi, 2, BigDecimal.ROUND_HALF_UP));
						disk.setType("G");
