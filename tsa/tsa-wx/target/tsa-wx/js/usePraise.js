...
function ajaxRefresh(currentPage){
	...
	cp=currentPage;
	Utils.ajax({
		url : 'praise/findMyPraiseList.action',
...
getHtml = function(data, currentPage) {
	...
	if (data.pageNums == 0) {
		...
		$.each(data.list,function(i, j) {
			...
			var state=j.state==1?"未发布":"已发布";
			tab1Html+="<div class=' bg-white mar-t6' onclick='jumpPage(\"praise/findPraiseDetail.action?id="+j.id+"&viewName=usePraiseDetail.jsp\")'>"
				+"<div class='ft12 pull-left ft14 ft-deep-blue mar-l10 hei-30'>"
				+"<i class='icon-number pull-left mar-t6'></i>"
				+"<span class='text-number pull-left mar-t6 '>"+j.title+"</span>"
				+"</div>"
				+"<div class='pull-right mar-r10'><i class='bg-green-merald tip-state'>"+state+"</i></div>"
				+"<p class='clearfix line-1eee'></p>"
				+"<a href='javascript:void(0);' class='weui-media-box weui-media-box_appmsg' style='padding-bottom: 8px;'>"
				+"<div class='weui-media-box__bd'>" //<i class='icon-position'></i>
				+"<h4 class='weui-media-box__title ft-bold400'>"+content+"</h4>"
...
function getBaseData(){
	$.showLoading();
	var viewName="usePraise.jsp";
	Utils.ajax({
		url:"praise/forwardPraise.action",
		data:{openid:openid,viewName:viewName,idNum:idNum},
		success:function(data){
			if(data.config!=null){
...
function usePraise(obj){
	...
	Utils.ajax({
		url : 'praise/insertPraise.action',
		data:{
			title:boardLocation,
			content:useCause,
			openid:openid,
...
getBackList = function(permitStr, orgCode) {
	...
	var type = $("#type").val();
	Utils.ajax({
		url : 'praise/findPraiseList.action',
		data : {
...
getBackHtml = function(data, currentPage) {
	...
	if (data.pageNums == 0) {
		...
		$.each(data.list,function(i, j) {
				...
				var state=j.state==1?"未发布":"已发布";
				tabHtml+="<div class=' bg-white mar-t6' onclick='jumpPage(\"praise/findPraiseDetail.action?id="+j.id+"&viewName=usePraiseDetail2.jsp\")'>"
					+"<div class='ft12 pull-left ft14 ft-deep-blue mar-l10 hei-30'>"
					+"<i class='icon-number pull-left mar-t6'></i>"
					+"<span class='text-number pull-left mar-t6 '>"+j.title+"</span>"
					+"</div>"
