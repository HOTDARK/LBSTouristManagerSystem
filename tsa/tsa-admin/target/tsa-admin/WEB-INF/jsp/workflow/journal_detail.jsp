...
<html>
		...
		<h5 class="flow-title"><i class="fi icon-flow"></i>${info.processName}</h5>
		  <ul class="detail-list"><li>日志ID:<span>${info.id}</span></li>
			...
			<li>流程结束时间:<span><fmt:formatDate value="${info.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></li>
		   </ul>
		</div>
	</div>
	<div class="row">
	...
	<div class="row">
		<div class="col-lg-12">
			<h5 class="result-title"><i class="fa fa-angle-right"></i>总结论:</h5>
			...
			<ul class="result-list">
				...
				<li>状态：
					<c:choose>
		            	<c:when test="${info.conclusionState=='normal' }"><span class="label label-info">正常</span></c:when>
		            	<c:when test="${info.conclusionState=='warn' }"><span class="label label-warning">警告</span></c:when>
		            	<c:when test="${info.conclusionState=='error' }"><span class="label label-danger">错误</span></c:when>
		            </c:choose>
				</li>
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="flow-step"><a class="btn btn-success" target="_blank" href="workflow/log/trace.do?id=${info.id}">点击查看</a></div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-12">
