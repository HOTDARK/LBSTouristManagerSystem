...
!function($) {
    ...
    /**
	 * 自定义扩展验证规则，每添加一个规格写明注释且必须先在这里标注出来：
	 * 1.调用名称：required			必填验证 
	 * 2.调用名称：number			数字验证
	 * 3.调用名称：mail				邮箱格式验证
	 * 4.调用名称：char				必须是英文字符		
	 * 5.调用名称：chinese			必须是汉字
	 * 6.调用名称：url				网址
	 * 7.调用名称：date				日期格式 hdX-XX-XX
	 * 8.调用名称：telephone			座机格式
	 * 9.调用名称：phone				手机格式
	 * 10.调用名称：ID				身份证格式
	 * 11.调用名称：account			账号验证
	 */
   $.fn.validation.defaults = {
        validRules : [
            ...
            },
            {
            	...
            	defaultMsg:'日期格式hdX-XX-XX'
            },
            {
            	name: 'telephone',validate:function(value){
            		return (!/^(\d{3,4})?\d{7,8}$/.test(value));
                },
            	defaultMsg:'座机号码输入格式有误'
            },
            {
            	name: 'phone',validate:function(value){
            		return (!/^(1\d{10})$/.test(value));
                },
            	defaultMsg:'手机号码输入格式有误'
		   },
		   {
			   name: 'ID',validate:function(value){
    ...
    var validateField = function(field, valid) { 
        ...
        if(form){
            ...
            if(fstyle == 0){
                ...
                controlGroup.find("#valierr").remove();
                el.parents('div.column').after('<span class="help-block" id="valierr">' + errorMsg +'</span>');
            }
        };//end !form
        return !error;
    };
                ...
                $(obj).find('input, textarea, select').each(function(){
                    ...
                    if (valid){
                        if ($.inArray('required',valid)>=0){
                            el.parents('div.column').after('<span class="help-block" id="valierr" style="color:#FF9966">*</span>');
                        }
                    };
                }); 
            };
